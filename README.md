...
We used CMake as our build system so the project can be built consistently across different machines and setups without having to manually configure a Visual Studio project every time. CMake generates the build files automatically based on the CMakeLists.txt configuration.
...
│   └── CMakeLists.txt       # CMake build config for the Nim source files
